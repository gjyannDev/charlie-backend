...
class AuthService:
    ...
    def login(self, user: UserLogin, db: Session) -> dict[str, str]:
        ...
        self.event_dispatcher.dispatch(
            UserLoggedIn(user_id=db_user.id, email=db_user.email)
        )
        return {
    ...
    def refresh(self, refresh_token: str, db: Session) -> dict[str, str | None]:
        ...
        self.event_dispatcher.dispatch(
            TokenRefreshed(user_id=db_user.id, email=db_user.email)
        )
        return {
    ...
    def logout(self, refresh_token: str, db: Session) -> dict[str, str]:
        db_token = self.token_repository.get_by_token(db, refresh_token, is_refresh=True)
        if not db_token:
